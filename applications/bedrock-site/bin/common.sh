#! /usr/bin/env bash

# get the project dir
PROJECT_NAME="bedrock";
PROJECT_VERSION=$1;
PROJECT_DIR="$(pwd)";
echo "Project: $PROJECT_NAME@v$PROJECT_VERSION";

# where are the sources
SRC_DIR="$PROJECT_DIR/src/main/javascript";

# the final product names
DEBUG_NAME="$PROJECT_NAME-debug-$PROJECT_VERSION.js";
RELEASE_NAME="$PROJECT_NAME-$PROJECT_VERSION.js";
DEBUG_NAME_FINAL="$PROJECT_NAME-debug.js";
RELEASE_NAME_FINAL="$PROJECT_NAME.js";

# the target names
TARGET_DIR="$PROJECT_DIR/target/js";
RELEASE_TARGET="$TARGET_DIR/$RELEASE_NAME";

# the artifact names
ARTIFACT_DIR="$PROJECT_DIR/src/main/webapp";
DEBUG_ARTIFACT="$ARTIFACT_DIR/js/$DEBUG_NAME";
RELEASE_ARTIFACT="$ARTIFACT_DIR/js/$RELEASE_NAME";
DEBUG_ARTIFACT_FINAL="$ARTIFACT_DIR/js/$DEBUG_NAME_FINAL";
RELEASE_ARTIFACT_FINAL="$ARTIFACT_DIR/js/$RELEASE_NAME_FINAL";
if [ ! -d "$ARTIFACT_DIR/docs/" ]; then
    mkdir "$ARTIFACT_DIR/docs/";
fi;
DOCS_ARTIFACT="$ARTIFACT_DIR/docs/$PROJECT_NAME-js";
